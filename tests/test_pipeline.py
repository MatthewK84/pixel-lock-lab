...
def test_sharpen_zero_is_passthrough(gray_frame: Array) -> None:
    assert apply_sharpen(gray_frame, 0.0) is gray_frame


def test_preprocessor_does_not_mutate_input(gray_frame: Array) -> None:
...
def test_preprocessor_disabled_is_passthrough(gray_frame: Array) -> None:
    processed = Preprocessor(PreprocessConfig(enabled=False)).apply(gray_frame)
    assert processed is gray_frame


def test_preprocessor_clahe_runs(gray_frame: Array) -> None:
...
def test_stabilizer_none_mode_is_passthrough(gray_frame: Array) -> None:
...
def test_stabilizer_imu_without_sample_is_passthrough(gray_frame: Array) -> None:
    result = Stabilizer(StabilizeConfig(mode=StabilizeMode.IMU)).apply(gray_frame, imu=None)
    assert result.applied_shift_px == (0.0, 0.0)


def test_stabilizer_imu_counteracts_motion(gray_frame: Array) -> None:
...
def test_stabilizer_clamps_to_max_shift(gray_frame: Array) -> None:
...
def test_stabilizer_visual_first_frame_is_passthrough(gray_frame: Array) -> None:
