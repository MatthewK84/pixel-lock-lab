...
def _seed_features(gray: np.ndarray, box: BoundingBox, max_points: int) -> np.ndarray:
    """Find trackable corners inside `box`, returned in full-frame coordinates."""
    mask: np.ndarray = np.zeros(gray.shape[:2], dtype=np.uint8)
...
def _flow_once(
    prev_gray: np.ndarray, next_gray: np.ndarray, points: np.ndarray
...
def _survivors(
    prev_gray: np.ndarray, next_gray: np.ndarray, points: np.ndarray, max_error_px: float
) -> np.ndarray:
    ...
    if backward.shape[0] == 0:
        return empty_points()
    error: np.ndarray = np.linalg.norm(points - backward, axis=2).reshape(-1)
    good: np.ndarray = (
...
class LucasKanadeTracker(BaseTracker):
    ...
    def __init__(self, config: TrackerConfig) -> None:
        ...
        self._points: np.ndarray = np.empty((0, 1, 2), dtype=np.float32)
        self._seed_count: int = 0

    def _initialize(self, frame: np.ndarray, bbox: BoundingBox) -> None:
        gray: np.ndarray = to_gray(frame)
        points: np.ndarray = _seed_features(gray, bbox, max_points=100)
    ...
    def _measure(self, frame: np.ndarray, _search_box: BoundingBox) -> Measurement:
        ...
        self._points = kept
        centroid: np.ndarray = kept.reshape(-1, 2).mean(axis=0)
    ...
    def _maybe_reseed(self, gray: np.ndarray, box: BoundingBox, score: float) -> None:
        ...
        if rate <= 0.0 or score > 0.75:
            return
        fresh: np.ndarray = _seed_features(gray, box, max_points=100)
